...
class FactualClaimExtractor:
    """I am extracting atomic factual claims from LLM responses."""
    ...
    def extract_claims(self, text: str) -> List[str]:
        ...
        """
        logger.info("I am extracting factual claims from the response...")
...
class FactVerifier:
    """I am verifying fact claims against retrieved evidence."""
    ...
    def verify_claim(self, claim: str, use_local: bool = True, use_web: bool = True) -> Dict[str, Any]:
        ...
        logger.info(f"I am verifying claim: {claim[:50]}...")
        
        evidence = []
        
        #local knowledge base first with enhanced queries
        if use_local:
            try:
                logger.info(f"--- Local Knowledge: I am checking the claim against PDF content ---")
                
                # trying multiple query strategies for better local retrieval
                queries_to_try = [
                    claim,  # Direct claim
                ...
                for query in queries_to_try:
                    try:
                        ...
                        #  check if we  got meaningful content (not just "I don't know" type responses)
        ...
        #  trying web search for additional evidence (only if local evidence is insufficient or as a fallback)
        if use_web and not local_evidence_found: # I only use web if local evidence wasn't conclusive
            try:
                logger.info(f"--- Web Search: I am searching for additional evidence ---")
                # extracting key terms from the claim for better search
                ...
                if web_result and "No relevant search results" not in web_result:
                    evidence.append({
                        'source': 'web_search',
                        'content': web_result,
                        'confidence': 0.7,  # assigning  lower confidence for web data
...
class RACCorrector:
    """I am the main RAC (Retrieval-Augmented Correction) system."""
    ...
    def correct_response(self, original_response: str, apply_corrections: bool = True) -> Dict[str, Any]:
        ...
        """
        logger.info("I am starting the RAC correction process...")
        
        # Step 1:  extract factual claims
        claims = self.claim_extractor.extract_claims(original_response)
        
        if not claims:
            logger.info("No factual claims extracted, I am returning the original response")
            return {
        ...
        if not use_local_source and not use_web_source:
            logger.warning("No verification sources enabled. I am skipping claim verification.")
            ...
            for i, claim in enumerate(claims, 1):
                logger.info(f"I am processing claim {i}/{len(claims)}: {claim[:50]}...")
...
class GoogleCustomSearchTool:
    """I am a tool to perform Google Custom Searches."""
    ...
    def search(self, query: str) -> str:
        ...
        """
        logger.info(f"--- Google Search: I am searching for '{query}' ---")
        params = {
            "key": self.api_key,
            "cx": self.cse_id,
            "q": query,
...
def curate_pdf_to_text(pdf_path, output_dir):
    ...
    logger.info(f"I am processing PDF: {os.path.basename(pdf_path)}...")
    full_text_pages = []
    try:
...
def load_single_document_for_indexing(file_path: str):
    ...
    if not os.path.exists(file_path):
        ...
        sys.exit(1)
    logger.info(f"I am loading document for indexing: {os.path.basename(file_path)}")
...
# --- Centralized MCP Processing Function ---
def process_mcp_query(
    ...
    try:
        ...
        if is_definitional_phrase and is_pdf_specific_query:
            ...
            # processed_question remains user_query
        ...
        if rac_corrector_instance.rac_enabled: # check RAC status from corrector instance if I manage it there
            response_trace.append("I am applying RAC (Retrieval-Augmented Correction)...")
            ...
            # Confidence Cascade Decision Logic
            ...
            response_trace.append("RAC Disabled, I am returning the agent's raw response.")
            return MCPResponse(
...
def main():
    ...
    logger.info("I am starting the Enhanced Hybrid Chatbot with RAC (Retrieval-Augmented Correction)...")
    ...
    logger.info("I am creating VectorStoreIndex for local PDF data...")
    try:
    ...
    def local_book_qa_function(query: str) -> str:
        ...
        """
        logger.info(f"--- Local RAG: I am querying for '{query}' ---")
    ...
    logger.info(f"I am initializing ReAct Agent with LLM: {OPTIMAL_LLM_MODEL_NAME}...")
    ...
    while True:
        ...
        if user_question.lower() == 'exit':
            logger.info("I am exiting the Enhanced Hybrid Chatbot. Goodbye!")
            break
        ...
        try:
            ...
            rac_stats['total_queries'] += 1 # Moved to process_mcp_query and then aggregated
            ...
            # You would ideally get granular stats from process_mcp_query result
            # For simplicity, I just use the global rac_stats, but for enterprise
            # I'd update based on mcp_response.confidence_score and other flags
